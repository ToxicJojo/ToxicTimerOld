...
$(document).ready(function () {
	$("#gameSelect").bind("change", function () {
		ToxicTimer.timer.UI.LoadRunList(parseInt(this.value));
	});
	...
	$("#runSelect").bind("change", function () {
		...
		ToxicTimer.timer.UI.ShowRun(parseInt(this.value));
	});
	...
	$("#fileInput").bind("change", ToxicTimer.timer.UI.LoadRunFromFile);
...
ToxicTimer.timer.UI.SwitchStartSplit = function (pSwitchTo) {
...
ToxicTimer.timer.UI.SwitchStopResume = function (pSwitchTo) {
...
ToxicTimer.timer.UI.LoadRunList = function (pGameId) {
...
ToxicTimer.timer.UI.LoadRunFromFile = function (pEvent) {
	ToxicTimer.io.LoadFile(pEvent, (function (pFileString) {
		...
		ToxicTimer.timer.UI.ShowRun(-1);
	}));
};

ToxicTimer.timer.UI.ShowRun = function (pRunId) {
	...
	if (ToxicTimer.currentRun !== undefined) {
		...
		for (var i = 0; i < ToxicTimer.currentRun.splits.length; i++) {
			ToxicTimer.timer.UI.ShowSplit(ToxicTimer.currentRun.splits[i]);
		}
...
ToxicTimer.timer.UI.ShowSplit = function (pSplit) {
...
ToxicTimer.timer.UI.UpdateSplit = function (pSplit) {
	if (pSplit !== undefined) {
